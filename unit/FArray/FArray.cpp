...
int main(int argc, char* argv[]) {
  ...
  yakl::init();
  {
    ...
    parallel_for( LoopBounds<1>(d1) , KOKKOS_LAMBDA (int i1) {
      test1d(i1) = 1;
    });
    parallel_for( LoopBounds<2>(d1,d2) , KOKKOS_LAMBDA (int i1, int i2) {
      test2d(i1,i2) = 1;
    });
    parallel_for( LoopBounds<3>(d1,d2,d3) , KOKKOS_LAMBDA (int i1, int i2, int i3) {
      test3d(i1,i2,i3) = 1;
    });
    parallel_for( LoopBounds<4>(d1,d2,d3,d4) , KOKKOS_LAMBDA (int i1, int i2, int i3, int i4) {
      test4d(i1,i2,i3,i4) = 1;
    });
    parallel_for( LoopBounds<5>(d1,d2,d3,d4,d5) , KOKKOS_LAMBDA (int i1, int i2, int i3, int i4, int i5) {
      test5d(i1,i2,i3,i4,i5) = 1;
    });
    parallel_for( LoopBounds<6>(d1,d2,d3,d4,d5,d6) , KOKKOS_LAMBDA (int i1, int i2, int i3, int i4, int i5, int i6) {
      test6d(i1,i2,i3,i4,i5,i6) = 1;
    });
    //parallel_for( LoopBounds<7>(d1,d2,d3,d4,d5,d6,d7) , KOKKOS_LAMBDA (int i1, int i2, int i3, int i4, int i5, int i6, int i7) {
    //  test7d(i1,i2,i3,i4,i5,i6,i7) = 1;
    //});
    //parallel_for( LoopBounds<8>(d1,d2,d3,d4,d5,d6,d7,d8) , KOKKOS_LAMBDA (int i1, int i2, int i3, int i4, int i5, int i6, int i7, int i8) {
    ...
    // Test host memset
    ///////////////////////////////////////////////////////////
    yakl::memset(testHost6d,0.f);
    if (yakl::intrinsics::sum(testHost6d) != 0) { die("memset: failed for testHost6d"); }

    ///////////////////////////////////////////////////////////
    // Test deep_copy_to device to host
    ///////////////////////////////////////////////////////////
    test6d.deep_copy_to(testHost6d);
    if (yakl::intrinsics::sum(testHost6d) != d1*d2*d3*d4*d5*d6) { die("deep_copy_to: wrong sum for testHost6d"); }

    ///////////////////////////////////////////////////////////
    // Test device memset
    ///////////////////////////////////////////////////////////
    yakl::memset(test6d,0.f);
    if (yakl::intrinsics::sum(test6d) != 0) { die("memset: failed for test6d"); }

    ///////////////////////////////////////////////////////////
    // Test deep_copy_to host to device
    ///////////////////////////////////////////////////////////
    testHost6d.deep_copy_to(test6d);
    if (yakl::intrinsics::sum(test6d) != d1*d2*d3*d4*d5*d6) { die("deep_copy_to: wrong sum for test6d"); }

    ///////////////////////////////////////////////////////////
    // Test createDeviceCopy from device
    ///////////////////////////////////////////////////////////
    auto test6d_dev2 = test6d.createDeviceCopy();
    if (yakl::intrinsics::sum(test6d_dev2) != d1*d2*d3*d4*d5*d6) { die("createDeviceCopy: wrong sum for test6d_dev2"); }

    ///////////////////////////////////////////////////////////
    // Test deep_copy_to device to device
    ///////////////////////////////////////////////////////////
    yakl::memset(test6d_dev2,0.f);
    test6d.deep_copy_to(test6d_dev2);
    if (yakl::intrinsics::sum(test6d_dev2) != d1*d2*d3*d4*d5*d6) { die("deep_copy_to: wrong sum for test6d_dev2"); }

    ///////////////////////////////////////////////////////////
    // Test slice
    ///////////////////////////////////////////////////////////
    yakl::memset(test6d,0.f);
    auto slice = test6d.slice<3>(COLON,COLON,COLON,4,3,2);
    yakl::memset(slice,1.f);
    if (yakl::intrinsics::sum(test6d) != d1*d2*d3) { die("slice: wrong sum for slice"); }
    ...
    // Test reshape
    ///////////////////////////////////////////////////////////
    auto reshaped = test6d.reshape<3>({{2,21},16,{-1,1132}});
    memset(reshaped,2.f);
    if (yakl::intrinsics::sum(test6d) != d1*d2*d3*d4*d5*d6*2) { die("SimpleBounds: wrong sum for reshaped test6d"); }


    ///////////////////////////////////////////////////////////
    // Test collapse
    ///////////////////////////////////////////////////////////
    auto collapsed = test6d.collapse(-1);
    memset(collapsed,3.f);
    if (yakl::intrinsics::sum(test6d) != d1*d2*d3*d4*d5*d6*3) { die("SimpleBounds: wrong sum for collapsed test6d"); }
